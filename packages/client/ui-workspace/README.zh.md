...
在 Tauri 桌面端中，用户可把外部文件（包括图片）拖入 Workspace 侧边栏。浏览器只把文件字节交给原生桌面壳，由其写入当前 Session 已关联的 Workspace 根目录。每次最多接收 64 个文件、单个文件最多 64 MiB、总量最多 256 MiB；文件名不能包含路径片段，遇到同名文件会追加数字后缀保留原文件。普通浏览器部署不提供原生导入，也不会把这些文件发送给服务器。
