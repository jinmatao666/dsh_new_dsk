...
在 Tauri 桌面端中，用户可把外部文件、图片和目录拖入当前 Session。Tauri 接收操作系统路径，并只允许页面消费这次拖入一次；原生壳把普通文件递归复制到 Session 的 Workspace，Session 没有 Workspace 时则复制到本地默认导入目录，并把顶层文件或目录作为标准文件／文件夹引用插入草稿。整次拖入最多接收 64 个文件、单个文件最多 64 MiB、总量最多 256 MiB；符号链接和特殊文件会被拒绝，顶层名称冲突时会追加数字后缀保留原条目。普通浏览器部署不提供原生导入，也不会把这些文件发送给服务器。
