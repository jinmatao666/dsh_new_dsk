...
function Empty({ icon, title, text, action }: { icon: 'history' | 'file'; title: string; text: string; action: () => void }) { return <div className={css.empty}><img className={css.icon} src={geologyIconImages[icon === 'file' ? 'files' : 'history']} alt="" aria-hidden="true" /><h3>{title}</h3><p>{text}</p><button type="button" className={css.primary} onClick={action}>新建纪要</button></div> }
