...
export type LandUsePlanReviewResult = {
  task: LandUsePlanReviewTask
...
export type LandUsePlanReviewTaskService = {
  list(): Promise<readonly LandUsePlanReviewTask[]>
  start(input: { name: string; files: readonly File[]; reviewCategory: number; coordinateSystem: string }): Promise<LandUsePlanReviewTask>
  read(task: LandUsePlanReviewTask): Promise<LandUsePlanReviewResult>
  wait(task: LandUsePlanReviewTask, onUpdate: (result: LandUsePlanReviewResult) => void): Promise<LandUsePlanReviewResult>
...
const skillSlug = 'market-gis-land-use-plan-review'

/** A dedicated archived Host session reuses the normal skill/model pipeline without a chat row. */
export function createLandUsePlanReviewTaskService(connection: ConnectionHandle, workspaces: IWorkspaces): LandUsePlanReviewTaskService {
  const native = async (command: string, args: unknown): Promise<unknown> => {
    ...
    if (invoke === undefined) throw new Error('请在桌面端使用土地利用规划审查专家。')
  ...
  const ensureSkill = async (): Promise<void> => {
    ...
    if (typeof skill?.id !== 'number') throw new Error('专家所需的土地利用规划审查技能未上架，无法开始分析。')
    const bundle = await remote('skill-bundle', { id: skill.id }) as { assets?: unknown; sha256?: unknown }
    if (typeof bundle.assets !== 'string' || typeof bundle.sha256 !== 'string') throw new Error('土地利用规划审查技能包不完整。')
    ...
    if (!Array.isArray(entries) || entries.length === 0) throw new Error('土地利用规划审查技能包没有文件。')
    const files = entries.map((entry) => {
      const file = entry as { path?: unknown; contentBase64?: unknown }
      if (typeof file.path !== 'string' || typeof file.contentBase64 !== 'string') throw new Error('土地利用规划审查技能文件格式无效。')
  ...
  const storageKey = async (): Promise<string> => {
    ...
    return `dsh.expert.land-use-plan-review.tasks.${value.username}`
  }
  const list = async (): Promise<readonly LandUsePlanReviewTask[]> => {
    const raw = localStorage.getItem(await storageKey())
    if (raw === null) return []
    try {
      const value: unknown = JSON.parse(raw)
      return Array.isArray(value) ? value.filter((task): task is LandUsePlanReviewTask => typeof task === 'object' && task !== null && typeof task.id === 'string' && typeof task.sessionId === 'string' && typeof task.directory === 'string') : []
    } catch { return [] }
  }
  const save = async (task: LandUsePlanReviewTask): Promise<void> => {
    const key = await storageKey()
    const existing = await list()
    localStorage.setItem(key, JSON.stringify([task, ...existing.filter(item => item.id !== task.id)]))
  }
  const read = async (task: LandUsePlanReviewTask): Promise<LandUsePlanReviewResult> => {
    ...
    return { task, status, answer, files, ...(analysisViewPath ? { analysisViewPath } : {}), ...(status === 'failed' ? { error: missing ? '本次规划审查未完整生成 Word 报告和 Excel 明细。' : '分析未正常完成，请查看上方模型回答。' } : {}) }
  }
  return {
    list,
    async start(input) {
      ...
      const task: LandUsePlanReviewTask = { id, sessionId, name: input.name, directory, createdAt: Date.now() }
      ...
      const prompt = `使用 /${skillSlug} 开展项目“${input.name}”的土地利用规划审查。输入范围：${inputPath}。输出目录：${directory}。审查类别 Blxsw：${input.reviewCategory || 4}。${input.coordinateSystem.trim() ? `用户确认的坐标系：${input.coordinateSystem.trim()}。` : ''}按该技能和审查类别完成真实接口分析，返回永久基本农田、建设分区、功能分区、冲突风险、需补充核实事项和数据限制，生成 Excel 明细和 Word 报告。明确说明结果属于规划合规研判，不是行政审批决定。不要复用已有分析结果；只报告本次实际生成的文件。若必须确认输入文件或坐标系，请说明原因并停止，不要猜测。`
