...
export type GeologyTaskService = {
  list(): Promise<readonly GeologyTask[]>
  start(input: { name: string; files: readonly File[]; fieldName: string; coordinateSystem: string }): Promise<GeologyTask>
  read(task: GeologyTask): Promise<GeologyResult>
  wait(task: GeologyTask, onUpdate: (result: GeologyResult) => void): Promise<GeologyResult>
...
const skillSlug = 'market-gis-geology-analysis'

/** A dedicated archived Host session reuses the normal skill/model pipeline without a chat row. */
export function createGeologyTaskService(connection: ConnectionHandle, workspaces: IWorkspaces): GeologyTaskService {
  const native = async (command: string, args: unknown): Promise<unknown> => {
    ...
    if (invoke === undefined) throw new Error('请在桌面端使用地质条件分析专家。')
  ...
  const ensureSkill = async (): Promise<void> => {
    ...
    if (typeof skill?.id !== 'number') throw new Error('专家所需的地质条件分析技能未上架，无法开始分析。')
    const bundle = await remote('skill-bundle', { id: skill.id }) as { assets?: unknown; sha256?: unknown }
    if (typeof bundle.assets !== 'string' || typeof bundle.sha256 !== 'string') throw new Error('地质条件分析技能包不完整。')
    ...
    if (!Array.isArray(entries) || entries.length === 0) throw new Error('地质条件分析技能包没有文件。')
    const files = entries.map((entry) => {
      const file = entry as { path?: unknown; contentBase64?: unknown }
      if (typeof file.path !== 'string' || typeof file.contentBase64 !== 'string') throw new Error('地质条件分析技能文件格式无效。')
  ...
  const storageKey = async (): Promise<string> => {
    ...
    return `dsh.expert.geology.tasks.${value.username}`
  }
  const list = async (): Promise<readonly GeologyTask[]> => {
    const raw = localStorage.getItem(await storageKey())
    if (raw === null) return []
    try {
      const value: unknown = JSON.parse(raw)
      return Array.isArray(value) ? value.filter((task): task is GeologyTask => typeof task === 'object' && task !== null && typeof task.id === 'string' && typeof task.sessionId === 'string' && typeof task.directory === 'string') : []
    } catch { return [] }
  }
  const save = async (task: GeologyTask): Promise<void> => {
    const key = await storageKey()
    const existing = await list()
    localStorage.setItem(key, JSON.stringify([task, ...existing.filter(item => item.id !== task.id)]))
  }
  const read = async (task: GeologyTask): Promise<GeologyResult> => {
    ...
    return { task, status, answer, files, ...(analysisViewPath ? { analysisViewPath } : {}), ...(status === 'failed' ? { error: missing ? '本次分析未完整生成 Word 报告和 Excel 明细。' : '分析未正常完成，请查看上方模型回答。' } : {}) }
  }
  return {
    list,
    async start(input) {
      ...
      const directory = await createExpertTaskDirectory(native, parent, `地质分析-${new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)}-${id.slice(0, 6)}`)
      ...
      const task: GeologyTask = { id, sessionId, name: input.name, directory, createdAt: Date.now() }
      ...
      const prompt = `使用 /${skillSlug} 分析项目“${input.name}”。输入范围：${inputPath}。输出目录：${directory}。分区名称字段：${input.fieldName || '分区名称'}。${input.coordinateSystem.trim() ? `用户确认的坐标系：${input.coordinateSystem.trim()}。` : ''}按该技能完成真实接口分析，返回综合结论、关键发现、项目影响与建议，生成 Excel 明细和 Word 报告。不要复用已有分析结果；只报告本次实际生成的文件。若必须确认输入文件或坐标系，请说明原因并停止，不要猜测。`
