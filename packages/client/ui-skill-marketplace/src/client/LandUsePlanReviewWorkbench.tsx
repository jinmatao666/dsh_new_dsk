    ...
    <main className={css.main}>{summaryError !== null && (section === 'history' || section === 'files') && <p className={css.error} role="alert">{summaryError}</p>}
      ...
      {section === 'workbench' && activeTask === null && <>
        {step === 'home' ? <HomeView onAction={startNew} onGuide={() => setSection('guide')} onHistory={() => setSection('history')} tasks={tasks} /> : <>
          ...
          </div> : <div className={css.reviewLayout}><section className={css.reviewCard}><h3>请核对这些信息</h3><dl><div><dt>项目名称</dt><dd>{projectName}</dd></div><div><dt>范围文件</dt><dd>{files.map(file => file.name).join('、')}</dd></div><div><dt>规划审查类别</dt><dd>{reviewCategory || '4'}</dd></div><div><dt>坐标系</dt><dd>{coordinateSystem || '从文件识别；无法确定时分析会说明'}</dd></div></dl><div className={css.reviewActions}><button type="button" className={css.secondaryButton} onClick={() =>{  setStep('prepare') }}>返回修改</button><button type="button" className={css.primaryButton} disabled={busy} onClick={() => void run()}>{busy ? '正在启动…' : '开始分析'}</button></div>{error !== null && <p className={css.error} role="alert">{error}</p>}</section><aside className={css.reviewAside}><strong>运行后将展示</strong><p>本次模型生成的综合结论、关键发现、项目影响与建议，以及实际生成的 Word 和 Excel 成果文件。</p><span>不会展示示例结论或不存在的文件。</span></aside></div>}</>}
      </>}
