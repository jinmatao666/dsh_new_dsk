...
import type { LandUsePlanReviewResult, LandUsePlanReviewTask, LandUsePlanReviewTaskService } from './land-use-plan-review-task.ts'
...
import { planReviewHero } from './LandExpertHeroImages.ts'

type LandUsePlanReviewIconName = 'mountain' | 'new' | 'workspace' | 'history' | 'files' | 'guide' | 'upload' | 'back' | 'layers' | 'coordinate' | 'geojson' | 'zip' | 'word' | 'excel' | 'result' | 'success' | 'failed' | 'running' | 'info' | 'help' | 'empty'
export function LandUsePlanReviewIcon({ name, className = '' }: { name: LandUsePlanReviewIconName; className?: string }) {
...
function outputFiles(result: LandUsePlanReviewResult | undefined): readonly string[] {
  return result?.files.filter(path => /\.(?:docx|xlsx)$/i.test(path)) ?? []
}

function ResultStatus({ result }: { result: LandUsePlanReviewResult | undefined }) {
...
/** Land-use planning review workspace; its service executes the fixed shipped skill through an archived session. */
export function LandUsePlanReviewWorkbench({ service, expertIcon, expertName, expertSubtitle }: {
  service?: LandUsePlanReviewTaskService
  ...
  const [reviewCategory, setReviewCategory] = useState('4')
  const [coordinateSystem, setCoordinateSystem] = useState('')
  const [tasks, setTasks] = useState<readonly LandUsePlanReviewTask[]>([])
  const [results, setResults] = useState<Record<string, LandUsePlanReviewResult>>({})
  const [activeTask, setActiveTask] = useState<LandUsePlanReviewTask | null>(null)
  const [busy, setBusy] = useState(false)
  const [advanced, setAdvanced] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [query, setQuery] = useState('')
  const [historyFilter, setHistoryFilter] = useState<'all' | LandUsePlanReviewResult['status']>('all')
  ...
  const openTask = async (task: LandUsePlanReviewTask) => {
    if (service === undefined) return
    setActiveTask(task)
  ...
  const run = async () => {
    ...
    setError(null)
    try {
      const task = await service.start({ name: projectName.trim(), files, reviewCategory: Number(reviewCategory) || 4, coordinateSystem })
  ...
  return <div className={css.shell}>
    <aside className={css.sidebar} aria-label="土地利用规划审查专家功能">
      <div className={css.identity}>
        <span className={css.identityMark}><ExpertProfileIcon icon={expertIcon ?? 'planning'} /></span>
        <div><strong>{expertName || '土地利用规划审查专家'}</strong><small>{expertSubtitle || '项目范围研判工作台'}</small></div>
      </div>
      <button type="button" className={css.newButton} onClick={startNew}>＋ 新建分析</button>
      <nav className={css.sideNav} aria-label="专家页面">
        {sections.map(item => <button key={item.id} type="button" className={section === item.id ? css.sideActive : ''} onClick={() => { setSection(item.id) }}><LandUsePlanReviewIcon name={item.id === 'workbench' ? 'workspace' : item.id} />{item.label}</button>)}
      </nav>
      <div className={css.sideFoot}><span className={css.statusDot} /> 土地利用规划审查<br /><small>记录和成果仅在此专家中查看</small></div>
    </aside>
    ...
    <main className={css.main}>{summaryError !== null && (section === 'history' || section === 'files') && <p className={css.error} role="alert">{summaryError}</p>}
      {section === 'workbench' && activeTask !== null && <>
        <div className={css.heading}><div><span className={css.kicker}>土地利用规划审查专家 · 分析任务</span><h2>{activeTask.name}</h2><p>创建于 {new Date(activeTask.createdAt).toLocaleString('zh-CN')}，结果和文件均来自本次任务。</p></div><button type="button" className={css.secondaryButton} onClick={() => void openTask(activeTask)}>刷新结果</button></div>
        ...
        <div className={css.resultLayout}>
          <div className={css.resultBody}>
            <section className={`${css.resultBanner} ${results[activeTask.id]?.status === 'failed' ? css.resultBannerFailed : results[activeTask.id]?.status === 'completed' ? css.resultBannerComplete : css.resultBannerRunning}`}>
              <LandUsePlanReviewIcon name={results[activeTask.id]?.status === 'failed' ? 'failed' : results[activeTask.id]?.status === 'completed' ? 'success' : 'running'} />
      ...
      {section === 'workbench' && activeTask === null && <>
        {step === 'home' ? <HomeView onAction={startNew} onGuide={() => setSection('guide')} onHistory={() => setSection('history')} tasks={tasks} /> : <>
          <div className={css.heading}><div><span className={css.kicker}>土地利用规划审查专家</span><h2>{step === 'prepare' ? '从项目范围开始' : '确认本次分析'}</h2><p>上传项目地块范围并填写项目信息，系统将调用正式规划审查技能核验规划关系与用途管制事项。</p></div></div>
          ...
          {step === 'prepare' ? <div className={css.columns}>
            <div className={css.primaryColumn}>
              <section className={css.formSection}><div className={css.sectionHeading}><span>01</span><div><h3>项目资料</h3><p>先给这次分析一个易于查找的名称。</p></div></div><label className={css.field}>项目名称<input value={projectName} onChange={(event) => { setProjectName(event.target.value); setError(null) }} placeholder="例如：东侧地块土地利用规划审查" /></label></section>
              <section className={css.formSection}><div className={css.sectionHeading}><span>02</span><div><h3>地块范围</h3><p>上传项目范围数据；提交分析后，技能会核查几何类型和坐标系。</p></div></div><input ref={fileInput} className={css.hiddenInput} type="file" accept=".geojson,.json,.zip,.shp,.shx,.dbf,.prj,.cpg" multiple onChange={(event) =>{  selectFiles(event.target.files) }} /><button type="button" className={css.uploadZone} onClick={() => fileInput.current?.click()} onDragOver={(event) =>{  event.preventDefault() }} onDrop={(event) => { event.preventDefault(); selectFiles(event.dataTransfer.files) }}><LandUsePlanReviewIcon name="upload" /><strong>{files.length > 0 ? '更换地块文件' : '点击或拖拽文件到此处上传'}</strong><small>支持 GeoJSON、完整 Shape ZIP，或同名的 .shp / .shx / .dbf 文件</small></button>{files.length > 0 && <div className={css.fileList}>{files.map(file => <span key={`${file.name}-${file.size}`}>{file.name}<small>{(file.size / 1024).toFixed(1)} KB</small></span>)}</div>}</section>
              <section className={css.advanced}><button type="button" aria-expanded={advanced} onClick={() =>{  setAdvanced(value => !value) }}>高级选项 <span>{advanced ? '−' : '＋'}</span></button>{advanced && <><label className={css.field}>审查类别（Blxsw）<input type="number" min="1" value={reviewCategory} onChange={(event) =>{  setReviewCategory(event.target.value) }} /><small>默认值为 4；仅在业务人员明确指定其他审查类别时修改。</small></label><label className={css.field}>坐标系说明（可选）<input value={coordinateSystem} onChange={(event) =>{  setCoordinateSystem(event.target.value) }} placeholder="例如 CGCS2000；文件有 .prj 时可留空" /><small>仅在文件中无法识别坐标系时填写，不能猜测。</small></label></>}</section>
              {error !== null && <p className={css.error} role="alert">{error}</p>}
            ...
            <aside className={css.infoColumn}>
              <section className={css.guidePanel}>
                <span className={css.guideIcon}><LandUsePlanReviewIcon name="geojson" /></span>
                <h3>准备地块数据</h3>
                <p>范围应为面或多面。GeoJSON、完整 Shape ZIP，或同名的 .shp / .shx / .dbf 文件均可提交。</p>
                <TerrainIllustration className={css.terrainArt} />
              </section>
              <section className={css.guidePanel}>
                <span className={css.guideIcon}><LandUsePlanReviewIcon name="result" /></span>
                <h3>本专家将交付</h3>
                <ul><li>规划符合性与用途管制审查</li><li>Word 专业分析报告</li><li>Excel 明细表</li></ul>
              </section>
              <p className={css.infoNote}>分析用于资料研判，不替代行政审批决定或最终法律结论。</p>
            </aside>
          </div> : <div className={css.reviewLayout}><section className={css.reviewCard}><h3>请核对这些信息</h3><dl><div><dt>项目名称</dt><dd>{projectName}</dd></div><div><dt>范围文件</dt><dd>{files.map(file => file.name).join('、')}</dd></div><div><dt>审查类别（Blxsw）</dt><dd>{reviewCategory || '4'}</dd></div><div><dt>坐标系</dt><dd>{coordinateSystem || '从文件识别；无法确定时分析会说明'}</dd></div></dl><div className={css.reviewActions}><button type="button" className={css.secondaryButton} onClick={() =>{  setStep('prepare') }}>返回修改</button><button type="button" className={css.primaryButton} disabled={busy} onClick={() => void run()}>{busy ? '正在启动…' : '开始分析'}</button></div>{error !== null && <p className={css.error} role="alert">{error}</p>}</section><aside className={css.reviewAside}><strong>运行后将展示</strong><p>本次模型生成的综合结论、关键发现、项目影响与建议，以及实际生成的 Word 和 Excel 成果文件。</p><span>不会展示示例结论或不存在的文件。</span></aside></div>}</>}
      </>}
      {section === 'history' && <div className={css.taskPage}><div className={css.heading}><div><span className={css.kicker}>土地利用规划审查专家</span><h2>我的分析记录</h2><p>仅显示当前用户在这台设备上创建的规划审查任务。</p></div></div><div className={css.statStrip}><span><LandUsePlanReviewIcon name="history" /><b>{tasks.length}</b><small>全部记录</small></span><span><LandUsePlanReviewIcon name="running" /><b>{tasks.filter(task => results[task.id]?.status === 'running').length}</b><small>分析中</small></span><span><LandUsePlanReviewIcon name="success" /><b>{tasks.filter(task => results[task.id]?.status === 'completed').length}</b><small>已完成</small></span><span><LandUsePlanReviewIcon name="failed" /><b>{tasks.filter(task => results[task.id]?.status === 'failed').length}</b><small>未完成</small></span></div>{tasks.length === 0 ? <div className={css.emptyWithAside}><EmptyView title="我的分析记录" description="仅展示本专家创建的分析任务。" detail="发起新分析后，相关记录将在这里显示。" steps={['上传地块范围', '核对输入信息', '查看分析结果']} onAction={startNew} /><aside className={css.emptySideCard}><LandUsePlanReviewIcon name="info" /><h3>记录范围说明</h3><strong>仅显示本专家创建的分析记录</strong><p>这里展示的是您使用「土地利用规划审查专家」创建的任务。其他专家的分析记录不会在此显示。</p></aside></div> : <><div className={css.listToolbar}><label className={css.searchField}>搜索项目<input value={query} onChange={(event) =>{  setQuery(event.target.value) }} placeholder="输入项目名称" /></label><div className={css.filterGroup} aria-label="筛选分析状态">{([['all', '全部'], ['running', '分析中'], ['completed', '已完成'], ['failed', '未完成']] as const).map(([value, label]) => <button type="button" key={value} aria-pressed={historyFilter === value} onClick={() =>{  setHistoryFilter(value) }}>{label}</button>)}</div></div>{loadingSummaries && <p className={css.listHint}>正在读取任务状态…</p>}<div className={css.taskList}>{filteredTasks.map(task => <button type="button" key={task.id} onClick={() => void openTask(task)}><span className={css.taskName}><strong>{task.name}</strong><small>创建于 {new Date(task.createdAt).toLocaleString('zh-CN')}</small></span><ResultStatus result={results[task.id]} /><span className={css.taskAction}>查看结果 ›</span></button>)}</div>{filteredTasks.length === 0 && !loadingSummaries && <p className={css.listHint}>没有符合当前搜索或筛选条件的分析记录。</p>}</>}</div>}
      {section === 'files' && <div className={css.taskPage}><div className={css.heading}><div><span className={css.kicker}>土地利用规划审查专家</span><h2>我的成果文件</h2><p>这里只列出本专家任务目录中实际找到的 Word 报告和 Excel 明细。</p></div></div><div className={css.statStrip}><span><LandUsePlanReviewIcon name="files" /><b>{listedFiles.length}</b><small>成果文件</small></span><span><LandUsePlanReviewIcon name="word" /><b>{listedFiles.filter(item => /\.docx$/i.test(item.path)).length}</b><small>Word 报告</small></span><span><LandUsePlanReviewIcon name="excel" /><b>{listedFiles.filter(item => /\.xlsx$/i.test(item.path)).length}</b><small>Excel 明细</small></span></div>{tasks.length === 0 ? <EmptyView title="我的成果文件" description="这里只展示本专家实际生成的成果文件。" detail="完成分析后，Word 报告与 Excel 明细会保存在这里。" steps={['完成地块分析', '生成 Word 报告', '打开 Excel 明细']} onAction={startNew} /> : <><div className={css.listToolbar}><label className={css.searchField}>搜索成果<input value={query} onChange={(event) =>{  setQuery(event.target.value) }} placeholder="输入文件名或项目名称" /></label><div className={css.filterGroup} aria-label="筛选文件类型">{([['all', '全部'], ['docx', 'Word 报告'], ['xlsx', 'Excel 明细']] as const).map(([value, label]) => <button type="button" key={value} aria-pressed={fileFilter === value} onClick={() =>{  setFileFilter(value) }}>{label}</button>)}</div></div>{loadingSummaries && <p className={css.listHint}>正在读取任务目录中的成果…</p>}{listedFiles.length === 0 && !loadingSummaries && summaryError === null ? <div className={css.noFiles}><span aria-hidden="true">▤</span><h3>暂无成果文件</h3><p>{query || fileFilter !== 'all' ? '没有符合当前搜索或筛选条件的文件。' : '当前任务目录中尚未找到 Word 报告或 Excel 明细。'}</p><button type="button" className={css.primaryButton} onClick={startNew}>新建分析</button></div> : <div className={css.fileTable}>{listedFiles.map(({ task, path }) => <div className={css.fileRow} key={`${task.id}:${path}`}><span className={css.fileGlyph} aria-hidden="true">{/\.docx$/i.test(path) ? 'W' : 'X'}</span><span className={css.fileIdentity}><strong>{path.split(/[\\/]/).at(-1)}</strong><small>{task.name} · {/\.docx$/i.test(path) ? 'Word 报告' : 'Excel 明细'}</small></span><button type="button" className={css.secondaryButton} onClick={() => void service?.openFile(path)}>打开文件</button></div>)}</div>}</>}</div>}
      {section === 'guide' && <div className={css.guidePage}><span className={css.kicker}>土地利用规划审查专家 · 使用说明</span><h2>准备一份完整的地块范围</h2><p>上传项目面范围，核对输入信息，再由分析技能生成结论与本次任务的 Word、Excel 成果。</p><div className={css.guideSteps}><span><b>1</b> 准备材料</span><span><b>2</b> 核对信息</span><span><b>3</b> 查看分析与交付</span></div><div className={css.guideCards}><section><h3>支持的材料</h3><p>面或多面的 GeoJSON、包含完整 Shape 文件的 ZIP，或同名的 .shp、.shx、.dbf 组件。若有 .prj，也请一并提供。</p></section><section><h3>坐标系与审查类别</h3><p>坐标系无法从文件识别时，请提供明确的坐标系说明。审查类别默认使用 4，只有明确业务依据时才在高级选项中修改。</p></section><section><h3>无法确认数据时</h3><p>如果存在多个候选数据集，或无法确定几何类型与坐标系，分析会说明需要补充的材料；请核对后新建任务。</p></section><section><h3>如何理解结果</h3><p>综合结论和建议来自本次模型回答；成果文件从本次任务目录读取。本分析用于前期资料研判，不替代行政审批决定或最终法律结论。</p></section></div></div>}
    </main>
  </div>
}
...
function HomeView({ onAction, onGuide, onHistory, tasks }: {
  ...
  onGuide: () => void
  onHistory: () => void
  tasks: readonly LandUsePlanReviewTask[]
}) {
  return <div className={css.homeLayout}>
    <div className={css.homeMain}>
      <section className={css.homeHero}>
        <img className={css.homeHeroImage} src={planReviewHero} alt="" aria-hidden="true" />
        <span className={css.kicker}>土地利用规划审查专家</span>
        <h2>土地利用规划审查专家</h2>
        <p className={css.homeSubhead}>规划符合性与用途管制审查</p>
        <div className={css.homeFeature}>
          <LandUsePlanReviewIcon name="layers" />
          <div>
            <strong>基于项目范围，形成专业分析</strong>
            <p>上传真实地块数据，识别规划关系条件与用途管制，查看分析结论和本次任务实际生成的成果文件。</p>
            <div className={css.featureTags}>
              <span>规划关系</span><span>用途管制</span><span>Word 报告</span><span>Excel 明细</span>
            </div>
          </div>
        </div>
    ...
    <section className={css.homeRecent}><header><div><h3>最近的审查记录</h3><p>这里展示本专家在当前设备上的任务。</p></div>{tasks.length > 0 && <button type="button" onClick={onHistory}>查看全部</button>}</header>{tasks[0] ? <button type="button" onClick={onHistory}><strong>{tasks[0].name}</strong><small>{new Date(tasks[0].createdAt).toLocaleString('zh-CN')}</small></button> : <div className={css.homeRecentEmpty}><LandUsePlanReviewIcon name="history" /><strong>暂无审查记录</strong><small>点击“新建分析”开始第一次审查。</small></div>}</section>
  </div>
}
...
function EmptyView({ title, description, detail, steps, onAction }: {
  ...
  return <div className={css.emptyPage}>
    <span className={css.emptyMark}><LandUsePlanReviewIcon name={title === '我的成果文件' ? 'files' : 'history'} /></span>
