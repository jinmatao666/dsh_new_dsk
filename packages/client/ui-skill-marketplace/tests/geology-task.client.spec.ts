...
import { createGeologyTaskService } from '../src/client/geology-task.ts'
...
describe('geology expert execution', () => {
  it('starts a fresh archived skill session and reads only its actual answer and output files', async () => {
    ...
    const history = vi.fn(async () => ({ result: { ok: true, value: { events: [
      { event: { type: 'assistant/message', data: { message: { content: [{ type: 'text', text: '本次地质分析结论' }] } } } },
    ...
    expect(result).toMatchObject({ status: 'completed', answer: '本次地质分析结论' })
    expect(result.files).toHaveLength(3)
    expect(result.analysisViewPath).toContain('analysis-view_20260923_120000_000.json')
