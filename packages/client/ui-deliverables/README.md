...
`deliverablesDefinition` folds each Turn's successful mutation calls into engine-published `DeliverablesTurnData`; `producedForClosing` reads that data with the closing Assistant seq. Mutation tools publish their follow-along `locations`. Office skills publish a `WANWEI_RESULT` JSON line whose `artifacts` array authoritatively names every created file; the browser reads it from the durable tool result, including output returned by `job_output` without a terminal result view. A result with `success: false` publishes none, and its diagnostic fields are not reinterpreted as output paths. The browser retains the explicit generated/saved-path fallback only for terminal generators without a valid office marker. None of these facts come from the closing prose, so a produced file is listed whether or not the model remembered to name it. A mutation is recognized by render intent, not tool name — a diff card, or a generic card whose `kind` is `edit` (the shape `str_replace_editor`'s insert presents) — so a new mutation tool joins by declaring what it does. Reads, deletes, and failed calls contribute nothing unless a successful result carries an office artifact marker; a path appears once per Turn in first-seen order. The Conversation Location index owns Turn membership, so a Turn that mutates and then ends without content text cannot spill into the next Turn's row.

`ProducedFiles` renders the row between the closing message's body and its IconActions footer: a quiet label and one measured file lane. It shows the largest leading prefix that fits (up to six chips; basename text, full path as the `title`) while reserving the exact localized `+ N files` width, so the remainder stays visible without wrapping or horizontal scrolling. Each chip opens through the owner-supplied `openFile` — the same Host opener the tool rows use, with the chat view resolving relative paths against the session cwd. A timestamped `*-analysis-view_*.json` from a bundled GIS skill is instead read through a filename- and size-restricted desktop command and rendered as a tabbed table card; its table has an independently scrolling body and an opaque sticky heading, so rows never paint through the headings while scrolling. The auxiliary JSON stays out of the ordinary file chips, while its matching Excel and Word files remain directly openable from the card. When files are hidden, a second-line **Show in folder** action opens the session workspace through that same owner path only while the page is loopback and the current Host handshake reports `canOpenPath`; direct remote Web and headless/container Linux Hosts omit the action by default. Design rationale: the [workspace file links Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md).
