...
该命令只由 Web bundle 挂载。只有 `/export` 返回成功时，本地 `command/executed` 确认才会在提交命令的浏览器中触发斜杠下载；其他标签页仍会渲染持久命令行，但不会重复执行浏览器副作用。Header 按钮直接调用同一个控制器。两种入口都会通过 `GET` 获取完整 ZIP：普通浏览器使用 Blob URL 交给下载管理器保存；桌面 WebView 将已获取的字节通过原生桥写入当前用户的“下载”文件夹。桌面桥会在外部 sidecar 的每个文档中安装，包括登录后的页面导航。两种入口共用并发折叠、插件释放时取消、准备阶段错误处理和同一个 Modal。

Host 下载端点会在 `readRaw` 前 flush 活动的根 Session，因此斜杠命令触发的 ZIP 会包含启动下载的 `command/run` 与 `command/done` 事件对。冷持久化 Session 不需要 flush。

弹窗报告准备中、已保存压缩包或失败。桌面端成功后会显示真实保存路径，并提供在操作系统文件管理器中定位 ZIP 的操作；普通浏览器仍由下载管理器接管。关闭弹窗不会取消正在进行的下载；该操作随后完成时也不会重新打开弹窗。每个 Session 同时只允许一项下载，重复操作会共用该任务。
...
- 浏览器下载由浏览器选择目标位置；桌面端导出固定写入当前用户的“下载”文件夹，不弹出目标位置选择。
- 控制器会在弹窗中报告 HTTP、ZIP 获取或原生保存失败。
