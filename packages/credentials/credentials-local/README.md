...
A desktop credentials document with the exact legacy `version: 1`, `refs`, and `records` wrapper is migrated on boot or before a write. The provider preserves the original as a uniquely named `.legacy-v1-*.yaml` backup beside the document, then writes the credential entries from `refs` in the flat format. Other invalid documents still fail validation.
