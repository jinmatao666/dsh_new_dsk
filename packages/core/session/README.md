...
Durable values need one accepted representation, not a check followed by a second read. `isJsonValue(value)` is the boolean predicate; `snapshotJsonValue(value)` iteratively validates and copies a plain value in one pass, returning `undefined` for invalid input and propagating a throwing getter. `stringifyJsonValue(value)` produces compact JSON from that vocabulary without invoking `toJSON`, so ambient prototype extensions cannot rewrite validated session metadata at persistence time. These helpers accept finite JSON numbers except `-0` (JSON rewrites it to `0`), dense ordinary arrays, and plain or null-prototype objects; they reject cycles, unsupported scalars, and exotic prototypes before normalization without imposing a traversal call-stack depth limit.
