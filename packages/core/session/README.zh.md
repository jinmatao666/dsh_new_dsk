...
持久值需要一种已接受的表示，不能先检查再二次读取。`isJsonValue(value)` 是布尔判断函数；`snapshotJsonValue(value)` 在一趟迭代中校验并复制普通值，无效输入返回 `undefined`，getter 抛出的异常则向外传播。`stringifyJsonValue(value)` 从该值域生成紧凑 JSON 且不调用 `toJSON`，因此环境中的原型扩展无法在持久化时改写已校验的会话元数据。这些辅助函数接受除 `-0` 外的有限 JSON 数值（JSON 会将其改写为 `0`）、稠密普通数组、普通对象或 null 原型对象；它们会在规范化前拒绝循环引用、不支持的标量和特殊原型，同时不对遍历施加调用栈深度限制。
