...
- 草稿承载纯文本，发出的文本由宿主边界评判——手动键入的手势、菜单 pick 与 TUI 提示词无从区分，也同样具有确定性。对于已识别的行首技能标识，编辑器在原生 textarea 旁显示缓存名称的行内标签，textarea 只编辑其后的正文；这样光标与可见正文对齐，而提交草稿仍保留 `/name`。其他位置的技能引用保持纯文本高亮。
