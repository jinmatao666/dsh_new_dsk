...
后一次拖入会替换待处理批次，复制开始前批次即被移除，因此页面不能重复消费，也不能自行提供任意源路径。该操作不会访问 One API，也不会持久化聊天附件。普通浏览器部署没有该桥接能力，并会提示直接导入不可用。复制完成后，顶层文件或目录会作为结构化引用插入输入框。按添加顺序排列的卡片负责显示文件名，不可见的编辑占位保留删除能力，并在发送时序列化为 Workspace 相对路径的 `@` 引用，文本区域不会重复显示文件名。
...
附件界面测试覆盖了原生拖入提示与一次消费。输入状态机测试固定了不可见文件占位及其剪贴板投影。原生测试固定了文件与目录递归复制、同名后缀和容量限制。
