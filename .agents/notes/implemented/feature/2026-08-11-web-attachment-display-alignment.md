...
Both overlays body-portal: the lightbox opened from a chat message sits under transformed ancestors that would trap `position: fixed` in their own box (the backdrop covered only the chat column), so `ImageLightbox` and `Toast` render through `createPortal(document.body)` and cover the viewport from every opener. The transient banner is a shared `ui-primitives` `Toast` atom: a compact translucent surface 20px from the viewport top, horizontally centered over its optional anchor, with a semantic status dot, single-line ellipsis, explicit close control, and `role="alert"`. It holds for three seconds, moves upward while fading for 420ms, pauses while hovered, and calls `onDone` when its lifetime ends. `InputBar` routes intake rejections (`addImages`'s returned reason) and `promptError` through it, `ModelSelect` routes rejected model selections through it, and skill marketplace operations use it instead of page-bottom result strips. Persistent or actionable contextual errors, including the model menu's catalog-load error with Retry, remain inline.
