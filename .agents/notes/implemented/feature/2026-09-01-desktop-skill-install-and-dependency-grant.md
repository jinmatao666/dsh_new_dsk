...
The NSIS preinstall and preuninstall hooks terminate the current and legacy desktop executable trees, then repeatedly terminate every remaining process whose executable path is inside the selected installation directory, including an orphaned bundled Node process. Closing the desktop window only hides it in the tray, so upgrade installation owns this shutdown instead of relying on the user to find the tray process. The hook waits up to 30 seconds for the installed Sharp DLL to become exclusively writable before extraction begins. If shutdown or write access still fails, installation stops with a Chinese diagnostic instead of partially replacing the runtime and surfacing a later NSIS file-write error.
