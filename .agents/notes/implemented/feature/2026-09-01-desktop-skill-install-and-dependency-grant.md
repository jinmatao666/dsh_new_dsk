...
The NSIS preinstall and preuninstall hooks terminate only processes whose executable paths are inside the selected installation directory, including an orphaned bundled Node process. They wait briefly for that product-local process set to exit but never use an exclusive Sharp DLL-handle probe as an install gate: antivirus scanning and access control can make that probe fail without an application lock. NSIS remains responsible for the actual replacement and surfaces a real write failure against the affected file.
