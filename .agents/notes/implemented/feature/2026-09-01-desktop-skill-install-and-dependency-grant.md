...
The OneAPI server owns the official skill catalogue and package bytes. At startup it creates missing records for the three bundled GIS skills and refreshes their bundled instructions, scripts, assets, metadata, and version while preserving each administrator-controlled publish and deletion state. The authenticated desktop host downloads a published package from the server; the renderer relays the package file set to a native Tauri command, which validates it in a same-volume `.dsh/.dsh-skill-staging/` directory outside the watched skill root and then atomically publishes it to the current user's `.dsh/skills/<slug>/` directory. The management browser obtains a complete dynamic skill record by ID, then decodes that same persisted package for its file tree, preview, and ZIP download instead of using the lightweight list record and synthesizing a single `SKILL.md`. The installer still carries the same packages as an offline recovery source, but normal installation and updates use the server copy. The filesystem skill provider watches only that user root, and the native command notifies the live renderer to clear its slash-menu catalog cache after every installation, update, or removal. The next `/` query in the existing conversation therefore discovers the changed catalog without a browser refresh. Installation state comes from the filesystem rather than browser storage. Updates and removal accept only a matching marketplace manifest or a recognized legacy marketplace `SKILL.md`; unrelated same-name user directories remain untouched.
...
The NSIS preinstall and preuninstall hooks register Sharp's native DLL with Windows Restart Manager, request shutdown of every reported holder, then terminate the current and legacy desktop executable trees plus every remaining process whose executable path is inside the selected installation directory, including an orphaned bundled Node process. Closing the desktop window only hides it in the tray, so upgrade installation owns this shutdown instead of relying on the user to find the tray process. The hook waits up to 30 seconds for the DLL to accept an exclusive read handle before extraction begins; that detects a live DLL handle without treating a write ACL as a lock. If a system-level holder still prevents replacement, installation stops before partial runtime replacement and directs the user to restart Windows.
