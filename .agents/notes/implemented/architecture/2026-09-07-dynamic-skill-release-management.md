...
The catalogue icon is administrator-owned presentation metadata. The first import initializes it from the package, with the built-in default as fallback; later package publication and rollback retain the current administrator-selected icon. Uploading a new package version therefore cannot reset a customized icon.
