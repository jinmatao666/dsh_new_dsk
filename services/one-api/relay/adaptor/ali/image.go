...
// Wan27ImageHandler：处理 wan2.7 系 multimodal 同步图像响应（一次返回，无需轮询）。
// 图片在 output.choices[].message.content[].image，转成 OpenAI ImageResponse 格式回写。
func Wan27ImageHandler(c *gin.Context, resp *http.Response) (*model.ErrorWithStatusCode, *model.Usage) {
	responseFormat := c.GetString("response_format")
	responseBody, err := io.ReadAll(resp.Body)
	if err != nil {
		return openai.ErrorWrapper(err, "read_response_body_failed", http.StatusInternalServerError), nil
	...
	var wanResp Wan27Response
	if err := json.Unmarshal(responseBody, &wanResp); err != nil {
		return openai.ErrorWrapper(err, "unmarshal_response_body_failed", http.StatusInternalServerError), nil
	}
	if wanResp.Code != "" {
		logger.SysError("wan2.7 image err: " + string(responseBody))
		return &model.ErrorWithStatusCode{
			Error:      model.Error{Message: wanResp.Message, Type: "ali_error", Code: wanResp.Code},
			StatusCode: resp.StatusCode,
		}, nil
	}

	imageResponse := openai.ImageResponse{Created: helper.GetTimestamp()}
	for _, choice := range wanResp.Output.Choices {
		for _, content := range choice.Message.Content {
			...
			if responseFormat == "b64_json" {
				if data, e := getImageData(content.Image); e == nil {
					b64Json = Base64Encode(data)
				} else {
					logger.SysError("wan2.7 getImageData err: " + e.Error())
					continue
				}
			}
			imageResponse.Data = append(imageResponse.Data, openai.ImageData{Url: content.Image, B64Json: b64Json})
		}
	}
