...
func RelayAudioHelper(c *gin.Context, relayMode int) *relaymodel.ErrorWithStatusCode {
	...
	// 企业账户:走企业账本扣减,不动 user_quota
	var orgDeducted map[int64]int64
	if useOrgQuota {
		// 四道闸门(总账本/成员累计上限/日月限额/部门预算)统一走公共校验
		if errResp := checkOrgSpendGuards(orgId, userId, orgDeptId, meta.OrgMemberQuotaLimit, preConsumedQuota); errResp != nil {
			return errResp
		}
		deducted, err := model.DecreaseOrgQuotaByLedger(orgId, preConsumedQuota)
		if err != nil {
			return openai.ErrorWrapper(err, "decrease_org_quota_failed", http.StatusInternalServerError)
		}
		orgDeducted = deducted
		if preConsumedQuota > 0 {
			if err := model.PreConsumeTokenQuotaOnly(tokenId, preConsumedQuota); err != nil {
				_ = model.RefundOrgQuotaByLedger(orgId, orgDeducted)
		...
		if userQuota-preConsumedQuota < 0 {
			return openai.ErrorWrapper(errors.New(getInsufficientQuotaMessage()), "insufficient_user_quota", http.StatusForbidden)
		}
	...
	defer func() {
		...
		if preConsumedQuota > 0 {
			...
			defer func(ctx context.Context) {
				go func() {
					if useOrgQuota {
						if err := model.PostConsumeTokenQuotaOnly(tokenId, -preConsumedQuota); err != nil {
							logger.Error(ctx, fmt.Sprintf("error rollback pre-consumed token quota: %s", err.Error()))
						}
						if err := model.RefundOrgQuotaByLedger(orgId, orgDeducted); err != nil {
							logger.Error(ctx, fmt.Sprintf("error rollback pre-consumed org quota: %s", err.Error()))
						}
					} else {
	...
	defer func(ctx context.Context) {
		go func() {
			if useOrgQuota {
				// 企业模式:仅调整 token 额度,不动用户个人额度
				if err := model.PostConsumeTokenQuotaOnly(tokenId, quotaDelta); err != nil {
					logger.Error(ctx, "error consuming token remain quota: "+err.Error())
				}
				// 账本差额 + 成员已用 + 部门用量 + 成员周期计数,统一走公共结算(含 0 下限钳制)
				postConsumeOrgLedger(ctx, orgId, userId, orgDeptId, quota, preConsumedQuota, orgDeducted)
				// 写日志(单独走,避免 PostConsumeQuota 内 user_quota 处理)
			...
			billing.PostConsumeQuota(ctx, tokenId, quotaDelta, quota, userId, channelId, modelRatio, groupRatio, audioModel, tokenName)
		}()
	}(c.Request.Context())
