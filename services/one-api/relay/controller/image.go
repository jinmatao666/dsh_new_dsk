...
func RelayImageHelper(c *gin.Context, relayMode int) *relaymodel.ErrorWithStatusCode {
	...
	if err != nil {
		...
		recordImageRelayErrorLog(c, meta, "", err.Error())
	...
	if bizErr != nil {
		recordImageRelayErrorLog(c, meta, imageRequest.Model, bizErr.Error.Message)
		return bizErr
	}

	imageCostRatio, err := getImageCostRatio(imageRequest)
	if err != nil {
		recordImageRelayErrorLog(c, meta, imageRequest.Model, err.Error())
	...
	if adaptor == nil {
		recordImageRelayErrorLog(c, meta, imageRequest.Model, fmt.Sprintf("invalid api type: %d", meta.APIType))
	...
	if meta.UseOrgQuota {
		// 企业账户:四道闸门(总账本/成员累计上限/日月限额/部门预算)统一走公共校验
		if errResp := checkOrgSpendGuards(meta.OrgId, meta.UserId, meta.OrgDeptId, meta.OrgMemberQuotaLimit, quota); errResp != nil {
			recordImageRelayErrorLog(c, meta, imageRequest.Model, errResp.Error.Message)
			return errResp
		}
	} else {
		if userQuota-quota < 0 {
			recordImageRelayErrorLog(c, meta, imageRequest.Model, getInsufficientQuotaMessage())
	...
	defer func(ctx context.Context) {
		...
		if meta.UseOrgQuota {
			// 企业账户:扣 token 额度 + 企业账本,日志带 OrgId
			if err := model.PostConsumeTokenQuotaOnly(meta.TokenId, quota); err != nil {
				logger.SysError("error consuming token remain quota: " + err.Error())
			}
			if quota > 0 {
				// image 为"post 一次性扣减"(无预扣),preConsumed=0、deducted=nil
				postConsumeOrgLedger(ctx, meta.OrgId, meta.UserId, meta.OrgDeptId, quota, 0, nil)
	...
	if respErr != nil {
		...
		// 仅在 deferred consume 不会写入时记录错误日志，
		// 避免同一请求同时产生 LogTypeConsume 与 LogTypeError 两条日志。
		// 该 defer 的写入条件是 resp 为 nil 或 status ∈ {200, 201}（见上方 defer 逻辑）。
		if resp == nil || (resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated) {
			recordImageRelayErrorLog(c, meta, imageRequest.Model, respErr.Error.Message)
		}
		return respErr
	}
