...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.AdminOperationLog()) // 后台操作审计记录
	{
		...
		skillRoute.Use(middleware.RequireRemoteSkills())
		{
			...
			skillRoute.POST("/:id/download", middleware.UserAuth(), controller.IncrementSkillDownloads)
			...
			skillAdminRoute.Use(middleware.AdminAuth())
			{
				...
				skillAdminRoute.POST("/import", controller.ImportSkillRelease)
			...
			skillWriteRoute.Use(middleware.AdminAuth())
			{
				...
				skillWriteRoute.POST("/:id/unpublish", controller.UnpublishSkill)
				skillWriteRoute.GET("/:id/releases", controller.ListSkillReleases)
				skillWriteRoute.POST("/:id/releases/:releaseId/validate", controller.ValidateSkillRelease)
				skillWriteRoute.POST("/:id/releases/:releaseId/publish", controller.PublishSkillRelease)
				skillWriteRoute.POST("/:id/releases/:releaseId/rollback", controller.RollbackSkillRelease)
