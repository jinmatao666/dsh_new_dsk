...
func SetRelayRouter(router *gin.Engine) {
	...
	// Usage reporting endpoint (TokenAuth only, no Distribute needed)
	usageRouter := router.Group("/v1/usage")
	usageRouter.Use(middleware.TokenAuth())
	{
		usageRouter.POST("/report", controller.ReportUsage)
	}

	relayV1Router := router.Group("/v1")
	relayV1Router.Use(middleware.RelayPanicRecover(), middleware.TokenAuth(), middleware.BindConfiguredWebSearchRoute(), middleware.Distribute(), middleware.SkillInject())
	{
