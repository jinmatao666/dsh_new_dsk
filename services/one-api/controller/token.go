...
// GetUserTokens 管理员查询指定用户的全部令牌（一次返回，不分页）。
func GetUserTokens(c *gin.Context) {
	...
	tokens, err := model.GetAllUserTokens(userId, 0, 1000, c.Query("order"))
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": err.Error(),
...
// AdminUpdateToken 管理员更新指定用户的某个令牌。支持 status_only 模式。
...
// AdminDeleteToken 管理员删除指定用户的某个令牌。
