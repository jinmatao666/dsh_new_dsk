...
func LarkBind(c *gin.Context) {
	...
	if model.IsLarkIdTakenByOther(user.LarkId, user.Id) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "该飞书账户已被绑定",
		})
	...
	// 阶段 5 单源化：lark_id 由账号中心持有,User.Update 已 Omit("lark_id")。
	if err := model.WriteAccountIdentifierByLocalUser(user.Id, model.IdentifierTypeLark,
		larkUser.OpenID, true, "lark_bind"); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": err.Error(),
		})
