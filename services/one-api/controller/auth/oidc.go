...
func OidcBind(c *gin.Context) {
	...
	if model.IsOidcIdTakenByOther(user.OidcId, user.Id) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	// 阶段 5 单源化：oidc_id 由账号中心持有,User.Update 已 Omit("oidc_id")。
	if err := model.WriteAccountIdentifierByLocalUser(user.Id, model.IdentifierTypeOidc,
		oidcUser.OpenID, true, "oidc_bind"); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": err.Error(),
		})
