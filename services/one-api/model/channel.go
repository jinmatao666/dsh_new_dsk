...
// GetChannelIdNameMap 返回全部渠道的 id → name 映射，用于给日志统计补齐渠道名。
// 渠道数量级很小（数十个），全量取出无压力。
func GetChannelIdNameMap() (map[int]string, error) {
	...
	var rows []row
	err := DB.Model(&Channel{}).Select("id, name").Find(&rows).Error
	if err != nil {
		return nil, err
	}
...
func (channel *Channel) Update() error {
	...
	// T1.4 ability 权威化:渠道保存不再「全删重建」ability(那会冲掉模型页配置的来源)。
	// 渠道页只负责同步「跟随渠道变化的属性」到该渠道已有的 ability 行:priority 与 enabled。
	// 模型↔渠道的增删关系由模型页(SyncAbilitiesForModel)独占维护。
	err = channel.SyncAbilitiesOnUpdate()
	return err
}
