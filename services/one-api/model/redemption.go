...
func Redeem(ctx context.Context, key string, userId int) (quota int64, err error) {
	...
	// 企业账户不持有个人积分,拒绝兑换
	var u User
	if err := DB.Select("id", "account_type").Where("id = ?", userId).First(&u).Error; err == nil && u.AccountType == AccountTypeEnterprise {
		return 0, errors.New("企业账户请联系企业管理员充值企业额度")
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		// 走永久定时积分账本(在事务内插行 + 同步 timed_quota_total / quota 镜像)
		row := UserTimedQuota{
			UserId:    userId,
			Amount:    redemption.Quota,
			Remaining: redemption.Quota,
			Source:    TimedQuotaSourceRedeem,
			SourceRef: key,
		}
		...
		if err := tx.Model(&User{}).Where("id = ?", userId).Updates(map[string]interface{}{
			"timed_quota_total": gorm.Expr("timed_quota_total + ?", redemption.Quota),
