...
func increaseTokenQuota(id int, quota int64) (err error) {
	// batch update 可能传入负值，用 GREATEST 兜底防止 remain_quota 为负
	err = DB.Model(&Token{}).Where("id = ?", id).Updates(
		map[string]interface{}{
			"remain_quota":  gorm.Expr("GREATEST(remain_quota + ?, 0)", quota),
...
func PostConsumeTokenQuota(tokenId int, quota int64) (err error) {
	...
	if quota > 0 {
		...
		// 消费回滚:落一笔永久定时积分(source=refund),不追溯原账本笔
		err = AddUserTimedQuota(token.UserId, -quota, TimedQuotaSourceRefund, "", nil)
	}
	if !token.UnlimitedQuota {
		if quota > 0 {
			err = DecreaseTokenQuota(tokenId, quota)
...
// PreConsumeTokenQuotaOnly 企业模式：只检查/扣减令牌额度，不检查用户个人额度
func PreConsumeTokenQuotaOnly(tokenId int, quota int64) error {
	if quota < 0 {
		return errors.New("quota 不能为负数！")
	}
	token, err := GetTokenById(tokenId)
...
// PostConsumeTokenQuotaOnly 企业模式：只调整令牌额度，不操作用户个人额度
func PostConsumeTokenQuotaOnly(tokenId int, quota int64) error {
	token, err := GetTokenById(tokenId)
	if err != nil {
		return err
	}
