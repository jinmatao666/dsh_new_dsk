...
> [!WARNING]
> 不是每一个主题都及时同步了所有功能。当前推荐并默认使用 `air` 主题，其他主题欢迎 & 期待 PR。
...
2. 把你的主题文件放到这个文件夹下。
3. 修改你的 `package.json` 文件，把 `build` 命令改为：`"build": "react-scripts build && mv -f build ../build/your-theme"`，其中 `your-theme` 为你的主题名。
