...
5. 脚本仅临时解压和转换 Shape 数据，结束后清理过程文件；Excel 与 Word 使用“输入文件名 + 分析名称 + 时间戳”的中文名称，不覆盖已有文件。原始 JSON、Markdown 底稿和对话分析视图用于追溯，不作为普通产物展示。Excel 分析表生成后会尝试用系统默认表格应用打开。

## 交付

- 原始分析结果 JSON。
- 对话分析视图 JSON，由桌面端自动渲染为可切换、可滚动的真实表格组件；不要把它作为普通产物向用户展示。
- Excel 分析表，包含“结果汇总”“专业分析”“接口明细”工作表，以结构化表格呈现研判结论和全部实际返回字段。
- Word 专业分析报告，详细说明输入核验、已确认字段的分析结果、记录数、面积或分区信息、专业研判和数据限制。
- Markdown 分析底稿。读取 `references/api.md` 与结果后，在对话中用中文给出详尽的地质环境分区、灾害易发性、面积构成、空间差异、建设影响和勘察建议；表格由结果卡片呈现，不再重复打印 Markdown 表格。只把 Excel 与 Word 作为主要交付文件列出。Excel 或 Word 任一文件未生成时，必须明确报告交付失败；接口名、字段代码和文件名保留原文。

`references/api.md` 是本技能的字段权威说明。不得对其中已定义字段反复提示“需要核实”或“待字段字典确认”。不同图层分别统计；不得相加不同图层的面积，每个比例必须写明本图层的统计口径。只有接口出现参考文档未定义的新字段时，才保留原字段名而不作推断。
