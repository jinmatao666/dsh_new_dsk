...
const PersonalSetting = () => {
  ...
  const getUserData = async () => {
    ...
    const { success, message, data, org_info } = res.data;
    if (success) {
            ...
            <Card
              ...
              footer={
                <Descriptions row>
                  {orgInfo ? (
                    <>
                      <Descriptions.Item itemKey="所属企业">{orgInfo.org_name}</Descriptions.Item>
                      <Descriptions.Item itemKey="剩余积分">
                        {orgInfo.quota_limit === -1 ? '不限' : renderQuota(orgInfo.quota_limit - orgInfo.used_quota)}
                      </Descriptions.Item>
                      <Descriptions.Item itemKey="已用额度">{renderQuota(orgInfo.used_quota)}</Descriptions.Item>
                    ...
                    <>
                      ...
                      <Descriptions.Item itemKey="订阅积分">{renderQuota(userState?.user?.subscription_quota ?? 0)}</Descriptions.Item>
                      <Descriptions.Item itemKey="定时积分">{renderQuota(userState?.user?.timed_quota_total ?? 0)}</Descriptions.Item>
