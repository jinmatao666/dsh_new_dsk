...
// 列配置元信息。key 对应列的 dataIndex;操作列恒显示。
const CHANNEL_COLUMN_META = [
  { key: 'id', label: 'ID' },
  { key: 'name', label: '名称' },
  { key: 'type', label: '类型' },
  { key: 'status', label: '状态' },
  { key: 'response_time', label: '响应时间' },
  // { key: 'expired_time', label: '已用/剩余' },
  { key: 'priority', label: '优先级' },
  { key: 'models', label: '测试模型' },
...
function renderType(type) {
  if (!type2label) {
    ...
    for (let i = 0; i < CHANNEL_OPTIONS.length; i++) {
      type2label[String(CHANNEL_OPTIONS[i].value)] = CHANNEL_OPTIONS[i];
    }
    type2label['0'] = { value: 0, text: '未知类型', color: 'grey' };
  }
  const item = type2label[String(type)] || type2label['0'];
  return <Tag size="large" color={item.color}>{item.text}</Tag>;
}
...
const ChannelsTable = () => {
  const columns = [
    ...
    // },
    {
      ...
      className: 'detail-column',
      render: (text, record, index) => {
        const models = text ? text.split(',').map(m => m.trim()).filter(Boolean) : [];
        ...
        return (
          <Select
            ...
            placeholder="选择测试模型"
            optionList={options}
            defaultValue={record.test_model || (models.length > 0 ? models[0] : undefined)}
            onChange={(value) => {
              record.test_model = value;
            }}
            filter
            size="small"
          />
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <div>
          ...
          <Popconfirm
            title="确定要复制此渠道吗？"
            content="将复制该渠道的全部配置，新渠道默认不启用"
            position={'left'}
            onConfirm={() => {
              copyChannel(record);
            }}
          >
            <Button theme="light" type="primary" style={{ marginRight: 1 }}>复制</Button>
          </Popconfirm>
          <Button theme='light' type='primary' style={{ marginRight: 1 }} onClick={() => testChannel(record, record.test_model || '')}>测试</Button>
          {
            record.status === 1 ?
  ...
  const { visibleColumns, columnConfigButton } = useColumnConfig({
    storageKey: 'channel_visible_cols',
    columnMeta: CHANNEL_COLUMN_META,
    allColumns: columns,
  });
  ...
  const copyChannel = async (record) => {
    try {
      // 复制在服务端完成：密钥仅在后端读取与写入，不经过前端（列表/详情接口均屏蔽 key）
      const res = await API.post(`/api/channel/copy/${record.id}`);
      const { success, message } = res.data;
      if (success) {
        showSuccess('渠道复制成功，已默认禁用！');
        await refresh();
      } else {
        showError(message);
      }
  ...
  return (
    <>
      ...
      <Table className={'channel-table'} style={{ marginTop: 15 }} columns={visibleColumns} dataSource={pageData} pagination={{
