...
export const CHANNEL_OPTIONS = [
  ...
  { key: 48, text: '讯飞星火 V2', value: 48, color: 'blue' },
  { key: 50, text: 'OpenAI 兼容', value: 50, color: 'green' },
  { key: 51, text: 'Gemini 兼容（OpenAI 协议）', value: 51, color: 'orange' },
...
CHANNEL_OPTIONS.sort((a, b) => a.value - b.value);

for (let i = 0; i < CHANNEL_OPTIONS.length; i++) {
  const { text, value } = CHANNEL_OPTIONS[i];
  CHANNEL_OPTIONS[i].label = (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
      <span>{text}</span>
      <span style={{ color: 'var(--semi-color-text-2)', marginLeft: 12 }}>{value}</span>
    </div>
  );
}
